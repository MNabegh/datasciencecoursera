...
motor_veichles_data <- NEI %>% filter(SCC %in% motor_veichles_sources & fips == "24510")
motor_veichles_data <- motor_veichles_data %>% group_by(year) %>% summarize(total = sum(Emissions)) 
...
plot <- motor_veichles_data %>% ggplot(aes(x = year, y = total))
print(plot + geom_col(fill = "Red") + ggtitle("Yearly total emissions from Motor Veichles Sources in Baltimore City")+
